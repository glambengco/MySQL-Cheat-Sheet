...
| Command    | Description                      | Example Query |
| ---------- | -------------------------------- | ------------- |
| `ORDER BY` | Sorts rows by a specified column. **NOTE:** Ascending is the default order, so no need to explicitly type `ASC` | `SELECT * FROM table1 ORDER BY age` - displays data sorted by increasing age |
|            | Use `DESC` for descending order  | `SELECT * FROM table1 ORDER BY salary DESC` - displays data sorted by decreasing salary |
| `WHERE`    | Filters data using a specific condition  | `SELECT name, job_position FROM table1 WHERE age >= 50` - displays names and job positions where age is at least 50 |
...
|            |                                  | `SELECT * FROM table1 WHERE name LIKE 'A__'` - displays rows where name starts with A and followed by exactly two characters |
...
| Command    | Description                                | Example Query           |
| ---------- | ------------------------------------------ | ----------------------- |
| `DISTINCT` | Displays unique values of a column         | `SELECT DISTINCT job_position FROM table1` - displays all unique values of job position |
...
| `SUM`      | Displays the sum of values from selection  | `SELECT SUM(salary) FROM table1` |
|            |                                            | `SELECT SUM(salary) FROM table1 WHERE age >= 60` - calculates total salary from all employees having age of at least 60 |
| `MIN`      | Displays minimum value from selection      | `SELECT MIN(salary) FROM table1` |
...
| `AVG`      | Calculates average value from selection    | `SELECT AVG(salary) FROM table1` |
| `STD`         | Calculates the population standard deviation from selection | `SELECT STD(salary) FROM table1` |
| `STDDEV`      | Same as `STD` |
| `STDDEV_POP`  | Same AS `STD` |
| `STDDEV_SAMP` | Calculates the sample standard deviation from selection | `SELECT STDDEV_SAMP(salary) FROM table1` |
| `VARIANCE`    | Calculates the population variance from selection | `SELECT VARIANCE(salary) FROM table1` |
| `VAR_POP`     | Same as  `VARIANCE` |
| `VAR_SAMP`    | Calculates the sample variance from selection | `SELECT VAR_SAMP(salary) FROM table1` |
...
| Command      | Description                       | Example Query                                               |
| ------------ | --------------------------------- | ----------------------------------------------------------- |
| `JOIN`       | Performs an inner join            | `SELECT * FROM table1 JOIN table2 ON table1.id = table2.id` |
| `LEFT JOIN`  | Performs a left outer join        | `SELECT * FROM table1 JOIN table2 ON table1.id = table2.id` |
| `RIGHT JOIN` | Performs a right outer join       | `SELECT * FROM table1 LEFT JOIN table2 ON table1.id = table2.id` |
| `CROSS JOIN` | Performs a cross join             | `SELECT * FROM table1 RIGHT JOIN table2 ON table1.id = table2.id` |
| `UNION`      | Performs a union of rows          | `SELECT * FROM table1 CROSS JOIN table2`                    |
...
### Using `WITH`  and `AS` for better readability:
```SQL
...
| Command        | Description | Example Query |
| -------------- | ----------- | ------------- |
| `OVER`         | Can be used to display aggregate function to a new column | `SELECT *, AVG(salary) OVER() FROM table1` - displays average salary to a new column |
| `PARTITION BY` | Used in `OVER` to group data when creating a Window function | `SELECT *, AVG(salary) OVER(PARTITION BY department) FROM table1` - displays average salary of the respective department of each row |
